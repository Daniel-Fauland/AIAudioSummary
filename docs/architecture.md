...
AIAudioSummary is a web app for uploading audio files (meeting recordings) and generating transcripts + AI-powered summaries. It uses:
...
- **Render** for deployment (private backend + public frontend with API proxy)
...
In production (Render), the backend is a **private service** — not reachable from the internet. The frontend is the only public service. All API calls from the browser go through a Next.js API proxy route.
...
Next.js Frontend (public, Render web service)
    ...
    └── /api/proxy/* ── forwards requests to backend
            │
            ▼
FastAPI Backend (private, Render private service)
...
CORS origins are configured via the `ALLOWED_ORIGINS` env var (comma-separated). Defaults to `http://localhost:3000` for local development. In production on Render the backend is a private service and only receives requests from the frontend's API proxy (server-to-server), so CORS is less relevant but still configured for safety.
...
```
Step 1: Upload                Step 2: Transcript              Step 3: Summary
┌─────────────────┐          ┌──────────────────────┐        ┌────────────┬────────────┐
│                 │          │  TranscriptView      │        │ Transcript │  Summary   │
│   FileUpload    │  ──►     │  (editable)          │  ──►   │ (readonly) │  (stream)  │
│  (drag & drop)  │          │  SpeakerMapper       │        │            │            │
│                 │          │  PromptEditor        │        │            │            │
└─────────────────┘          │  [Generate] button   │        └────────────┴────────────┘
                             └──────────────────────┘              Two-column grid
```
...
- **1 -> 2**: `handleFileSelected()` — uploads file, calls `/createTranscript`
...
Currently used: `badge`, `button`, `card`, `dialog`, `input`, `label`, `select`, `separator`, `sheet`, `sonner`, `switch`, `tabs`, `textarea`.
...
User drops audio file
    │
    ▼
FileUpload validates file type (audio/video extensions)
    │
    ▼
...
3. Render will detect the `render.yaml` and show the two services:
   - **aias-backend** (Private Service)
   - **aias-frontend** (Web Service)
4. Click **Apply**
...
5. `BACKEND_INTERNAL_URL` is auto-populated from the backend private service
...
| Backend unreachable | Verify the backend is running (check Render logs). Ensure it's a **Private Service** (type `pserv`), not a Web Service |
